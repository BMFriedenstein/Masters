...
\section{Develop and verify a simulation model}
	...
	\par 
	The simulation step size indicates the data resolution. A lower step size will result in a more accurate simulation of the system, however the processing and data analysis is effected. In this study, the smallest available step size is selected to ensure that the the simulated results achieve the desired precision. Higher step sizes (30+ min.) complicates control models as precisely replicating quick changes such as opening/closing valves or compressors stopping or staring etc. becomes more difficult.
...
\section{Implementation of simulation method}
	...
	\subsection{Quantify operational benefit}
		...
		\begin{figure}[h]
			...
			\caption[Eskom's weekday TOU tariff structure.]{Eskom's weekday \gls{tou} tariff structure.}
			\label{fig: Tariff}
		\end{figure}
