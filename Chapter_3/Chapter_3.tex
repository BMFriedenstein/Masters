...
\section{Investigate the system}
	...
	\end{itemize} 
	\par 
	Therefore, a data verification methodology is utilised to ensure that datasets are of a high quality. In this study, a dataset is built comprising of all the available mining sensors for a 24 hour period of the systems operation. In this study the following checks are performed to verify a data-sets quality:
	\begin{enumerate}
		\item Is there missing patches in data?
		\item Are the measured values spiking or stagnating?
		\item Do the values fall in the expected range and follow a typical trend identified from historical data? For example if a compressor has a maximum power rating 4 $MW$ and historical data has shown the machine has operated between 3 and 4 $MW$. Values outside this range could indicate bad readings.
	...
	\end{enumerate} 
If the dataset passes all the checks, or if the failed check are explainable or can be corrected, the dataset is regarded as of high enough standard to be used in the simulation.
...
\section{Develop and verify a simulation model}
	...
	\subsection{Model the air network components}
		...
		\subsubsection{Compressed air controllers}\label{Controllers}
			...
			\par 
			On a typical mine, a compressor's power output is controlled to ensure that the discharge pressure matches a specified  set-point. This control is achieved through either \glspl{vsd} (or \glspl{vfd}) or guide vane control. On \gls{ptb}, valve or compressor control can be replicated using a \gls{pi} controller as shown in \Cref{fig: Controller models}. For the control system models in \Cref{fig: Controller models}, outlet pressure is used as feedback to the compressor and a valve controller. 
			\par 
...
\section{Implement the simulation}
	...
	\subsection{Quantify operational benefit}
		With the data for each of the simulated scenarios exported, the relative improvement compared with the baseline should now be quantified. This comparison is achieved by analysing the differences between the theoretical baseline and optimised data series as shown visually in \Cref{fig: Savings Power.}. The theoretical improvement was achieved by reducing the power of baseline data series between 16:00 and 00:00. To quantify the benefit, the saving series is calculated by subtracting the the optimised series from the baseline:
