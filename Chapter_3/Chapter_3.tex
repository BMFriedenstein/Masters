...
This chapter details the development and implementation methodology of simulations to optimise mining compressed air systems. The method developed uses insights gathered from the literature reviewed in \Cref{Beet verification table}.
\par 
...
\section{Develop and verify a simulation model}
	...
	The simulation boundaries determine the detail that the system process is modelled. For a simple compressed air model, the boundaries can be set around the compressor house. This model would then only include the compressor components, inlet and outlet air flows. Alternatively, a more complex model can be developed by choosing boundaries to include more aspects of the process such as specific flows on mining levels.
	 ...
	 \begin{figure}[h]
	 	\centering
	 	\fbox{\hspace{0.125\textwidth}\includegraphics[trim=0 3cm 0 0, width=0.75\textwidth]{Images/3/Detail.pdf}\hspace{0.125\textwidth}}
	...
	\subsection{Model compressed air network components}
		...
		\subsubsection{Demand}
			...
			\begin{figure}[h]
				\centering
				\fbox{\includegraphics[trim =-12cm 0.5cm -12cm 0.5cm, width=\textwidth]{Images/3/AirDemand}}
	...
	\clearpage
		\begin{figure}[h]
			\centering
			\fbox{\includegraphics[trim =1.0cm 3.9cm 0.76cm 1cm,width=\textwidth]{Graphs/4/PeriodicProcess/PeriodicProcess.pdf}}
