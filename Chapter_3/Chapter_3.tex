...
\section{Develop and verify a simulation model}
	...
	The simulation boundaries determine the detail that the system process is modelled. For a simple compressed air model, the boundaries can be set around the compressor house. This model would then only include the compressor components, inlet and outlet air flows. Alternatively, a more complex model can be developed by choosing boundaries to include more aspects of the process such as specific flows on mining levels.
	 ...
	 \begin{figure}[h]
	 	\centering
	 	\fbox{\hspace{0.125\textwidth}\includegraphics[width=0.75\textwidth]{Images/3/Detail.pdf}\hspace{0.125\textwidth}}
	...
	\end{figure}
		...
		\end{figure}
		A guide vain controller is modelled using a \gls{pi} controller. However, the non linear limitations of guide vain control, as represented in \cref{fig: Guide vain position}, must be implemented in the controller. This is done by using a minimum control output limit that matches the minimum power reduction achieved by closing the guide vain to its minimum position. For example, a \gls{pi} controller for the compressor from \cref{fig: Guide vain position} would have a minimum control output of approximately 60\%.
		\par 
		...
		\begin{figure}[h]
			\centering
			\fbox{\includegraphics[trim =-1.5cm 0 -1.5cm 0cm, width=\textwidth]{Images/3/Valve.jpg}}
