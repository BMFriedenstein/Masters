...
\begin{figure}[!htbp]
	\centering
	\fbox{\hspace{3.5cm}\input{Images/2/Compressor/compressor}\hspace{2.4cm}}
...
\section{Simulation software tools}
...
The equation is rewritten to get a relative error percentage:	
 ...
 \par
 The results of other verification methods applied on the example are provided in \Cref{Philip verification table}. From the table it is clear that the accuracy of the simulation model is not as high as interpretred using the residual difference alone.
 \begin{table}[!htbp]
 	\centering
 	\begin{tabular}{lcr}
 		\hline
...
\subsection{Preamble}
This section will review literature regarding simulation usage in optimising compressed air systems. Shortcomings identified from the literature that can be improved upon in this study will be addressed. The studies that were reviewed in this section are summarised in table \Cref{table: cm sim studies}.
	\begin{table}[!htbp]
	\centering
	\begin{tabular}{p{5cm}ccl}
		\hline
...
\end{figure}
The approach to determining the optimum compressor location would not be suitable for mining compressed air system. Mining air networks extend for kilometres in the x, y and z planes. The study greatly simplified the compressed air network in order to easily formulate the mathematical model. This approach would not be suitable for larger and more complex mining air networks.
