...
\end{itemize} 
 \gls{ptb} was used for the study. The software design allows simpler and speedier development of system models, because many components such as compressor mathematical models have already been pre-built \cite{Mare2016PhD}. 
 % rems, DCS
 \subsubsection{PTB background}
 \gls*{ptb} is a transient system simulation and optimisation tool and the software was designed for use in building thermal hydraulic systems. In a recent study, Maré states that \gls{ptb} can easily be applied to mining systems \cite{Mare2016PhD}.  \gls{ptb} provides a simple graphical user interface to develop system models by dragging and dropping components intuitively. The interface is shown in \Cref{fig: PTB UI}.
 \par 
