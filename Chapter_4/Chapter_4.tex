...
\par 
	...
	\begin{figure}[h!]
		\centering
		\fbox{\includegraphics[trim =0cm 0cm 0cm 0cm,width=\textwidth]{Graphs/4/LevelLayout/105L2.pdf}}
	...
	From  investigations that were conducted on the compressed system, refuge bays leaks were identified as a significant inefficiency that can be reduced. Early tests on a single mining level indicated that closing refuge bay leaks would lead to a reduction of "$100m^3/hr$" per refuge bay. This measurement was conservative as not all of the refuge bays on the level could be closed for the test.
	\par 
	Further test, integrating all refuge bays in mine for a longer period would not be possible without buy in from the mine's management. To gain this buy-in, the potential financial and operational benefits would need to first be quantified. A simulation simulation model was hence developed to determine this.
	\par
	Refuge bay components were added to the baseline modelled discussed previously. The component was modelled as a leak using the measurements from the tests. Using the per-level layouts obtained from the investigation the location and number of refuge bay components ere identified. The full simulation model is shown in Figure \ref{fig: Refuge bay layout} in Appendix \ref{ASchematics}.
